...
BLYNK_WRITE(V1)  { integraleWattSecondo = param.asDouble(); }
BLYNK_WRITE(V50) { banda = param.asInt()/100.0; }
BLYNK_WRITE(V51) { scarto = param.asInt(); }
//BLYNK_WRITE(V52) { BandaMin = param.asInt(); }
//BLYNK_WRITE(V53) { BandaMax = param.asInt(); }
BLYNK_WRITE(V54) { K_low = param.asInt()/1000.0; }
BLYNK_WRITE(V55) { K_high = param.asInt()/1000.0; }
...
void fun()
{
  ...
  TEL_SER.print("FiltMin: "+String(FiltMin)+"\t");
  TEL_SER.print("FiltMax: "+String(FiltMax)+"\t");
...
void handleGraph()
{
  Serial.println("Sending data to the web!");
  server.setContentLength(CONTENT_LENGTH_UNKNOWN);
  server.sendHeader("Cache-Control","no-cache");
  server.sendHeader("Transfer-Encoding","chunked");
  server.send(200,"image/svg+xml");
  //server.sendContent("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 "WIDTH" 1024\"> preserveAspectRatio=\"none\"\n");
  // 2140 = (200-1+15)*10
  ...
  for(int i=1; i<400/CHUCNKSIZE; i++) {
    ...
    //delay(20);
    Serial.println(strlen(chunk));
  }
  server.sendContent("</g>\n</svg>\n");
  server.sendContent("");
}
